  ...
  return (
    <div className="service-detail">
      ...
      <section className="section section--dark service-process-section">
        <div className="container"><SectionHeading eyebrow="04 / Process" onDark title="HOW WE WORK" /><div className="service-process service-process--industrial" style={{ marginTop: '2rem' }}>{PROCESS_STEPS.map((step, i) => <Reveal key={step.number} dir="up" delay={i * 80}><article className="service-process__step"><span>{step.number}</span><div><h3>{step.title}</h3><p>{step.note}</p></div></article></Reveal>)}</div></div>
      </section>

      <section className="section section--light-green service-gallery-section">
        <div className="container"><div className="service-gallery__heading"><SectionHeading eyebrow="05 / Gallery" title="VISUAL MOMENTS" /><p>Selected frames from the ATTI VERSE production world.</p></div><div className="service-gallery service-gallery--industrial" style={{ marginTop: '2rem' }}>{gallery.map((src, i) => <Reveal key={src + i} dir="up" delay={i * 80}><div className={`service-gallery__item ${i === 0 ? 'service-gallery__item--large' : ''}`}><Img src={src} alt={`${service.title} — visual ${i + 1}`} /><span>0{i + 1} / {service.lens}</span></div></Reveal>)}</div></div>
      </section>
